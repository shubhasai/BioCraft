...
import com.shubhasai.biocraft.databinding.FragmentMarketmasterBinding
...
class MarketmasterFragment : Fragment(),EducationAdapter.EdcDelClicked,WorkExperienceAdapter.WorkDelClicked
    ...
    private lateinit var binding:FragmentMarketmasterBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentMarketmasterBinding.inflate(layoutInflater)
        ...
        viewModel.getallLangs().observe(viewLifecycleOwner) { language ->
            if(language!= null){
                ...
                for(data in language){
                    ...
                    chip.setChipBackgroundColorResource(R.color.mmred)
...
// Create each page
        for (currentPage in 0 until totalPages) {
            val pageInfo = PdfDocument.PageInfo.Builder(view.width, view.height, currentPage + 1).create()
